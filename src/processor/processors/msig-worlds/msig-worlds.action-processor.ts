...
import { Actions } from '@alien-worlds/msig-worlds-common';
...
> {
  public async run(model: ActionTraceProcessorModel<ContractData>): Promise<void> {
    try {
      ...
      const mongoMapper = new Actions.Mappers.MsigWorldsActionMongoMapper();
      const source = new Actions.MsigWorldsActionMongoSource(dataSource as MongoSource);
      const repository = new RepositoryImpl(
        source,
        mongoMapper,
        new MongoQueryBuilders(mongoMapper)
      );
      const taskModelMapper = new Actions.Mappers.MsigWorldsActionProcessorTaskMapper();
